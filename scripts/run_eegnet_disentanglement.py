#!/usr/bin/env python3
"""Run cropped end-to-end EEGNet stimulus-adversarial residual models."""
...
from openaffect_eeg.eegnet import (
    EEGNetTrainingConfig,
    fit_adversarial_eegnet,
    fit_adversarial_eegnet_fixed_epochs,
    prepare_eegnet_normalization,
)
from openaffect_eeg.raw_trials import load_raw_trial_archive
from openaffect_eeg.statistics import group_eta_squared

DEFAULT_STRENGTHS = (0.0, 0.1, 0.5)
ZERO_MODEL = "eegnet_zero_adversary_prior_plus_eeg_residual"
SELECTED_MODEL = "eegnet_selected_adversary_prior_plus_eeg_residual"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("tensor_archive", type=Path)
    parser.add_argument("trial_uid_archive", type=Path)
    ...
    parser.add_argument("--temporal-kernel", type=int, default=51)
    ...
    parser.add_argument("--f2", type=int, default=4)
...
def select_adversary(
    ...
    *,
    ...
    train_indices = train["tensor_index"].to_numpy(dtype=int)
    validation_indices = validation["tensor_index"].to_numpy(dtype=int)
    ...
    normalization = prepare_eegnet_normalization(
        tensors, train_indices, train_residual
    )

    scores: dict[float, dict[str, float | int]] = {}
    for strength in strengths:
        fitted = fit_adversarial_eegnet(
            tensors,
            train_indices,
            train_residual,
...
def identity_audit(values: np.ndarray, trials: pd.DataFrame) -> dict[str, float]:
    return {
        "stimulus_eta_squared": group_eta_squared(
            values, trials["stimulus_uid"].astype(str).to_numpy()
        ),
        "subject_eta_squared": group_eta_squared(
            values, trials["subject_id"].astype(str).to_numpy()
        ),
    }
...
def run_assignment(
    ...
    *,
    ...
    fit_indices = fit["tensor_index"].to_numpy(dtype=int)
    test_indices = test["tensor_index"].to_numpy(dtype=int)
    ...
    normalization = prepare_eegnet_normalization(
        tensors, fit_indices, fit_residual
    )
    ...
    zero = fit_adversarial_eegnet_fixed_epochs(
        tensors,
        fit_indices,
        fit_residual,
    ...
    if selected_strength == 0.0:
        ...
        selected = fit_adversarial_eegnet_fixed_epochs(
            tensors,
            fit_indices,
            fit_residual,
            fit["stimulus_uid"].astype(str).to_numpy(),
    ...
    zero_residual = zero.predict(tensors, test_indices)
    selected_residual = selected.predict(tensors, test_indices)
    models = {
    ...
    result: dict[str, object] = {
        ...
        "latent_identity_audit": {
            "zero_adversary": identity_audit(
                zero.transform(tensors, test_indices), test
            ),
            "selected_adversary": identity_audit(
                selected.transform(tensors, test_indices), test
            ),
        },
        "residual_identity_audit": {
            "zero_adversary": identity_audit(zero_residual, test),
            "selected_adversary": identity_audit(selected_residual, test),
        },
        "models": {
...
def main() -> int:
    ...
    trial_uids, tensors = load_raw_trial_archive(
        args.tensor_archive, args.trial_uid_archive
    )
    trials = harmonize_affect_targets(pd.read_csv(args.trial_table, sep="\t"))
    trials = trials.loc[trials["dataset_id"].eq("ds005540")].copy()
    trials["tensor_index"] = trials["trial_uid"].map(
        {trial_uid: index for index, trial_uid in enumerate(trial_uids)}
    )
    trials = trials.loc[
        trials["target_available"] & trials["tensor_index"].notna()
    ].copy()
    trials["tensor_index"] = trials["tensor_index"].astype(int)
    if len(trials) != len(tensors):
        raise ValueError("Raw tensor archive does not exactly cover supervised trials")
    ...
    for protocol in requested_protocols:
        for path in sorted((args.split_root / protocol).glob("seed-*.tsv.gz")):
            if requested_seeds is not None:
                assignment_seed = int(
                    path.name.removeprefix("seed-").removesuffix(".tsv.gz")
                )
                if assignment_seed not in requested_seeds:
                    continue
    ...
    if args.predictions_output is not None:
        ...
        if predictions.duplicated(["protocol", "seed", "trial_uid"]).any():
    ...
    payload = {
        "dataset_id": "ds005540",
        "objective": (
            "train-only stimulus prior plus cropped end-to-end EEGNet "
            "experienced-affect residual with stimulus gradient reversal"
        ),
        ...
        "crop_policy": (
            "One deterministic random crop per train trial and epoch; test "
            "prediction averages all non-overlapping crops plus a final tail crop"
        ),
        "selection_policy": (
        ...
        "zero_adversary_policy": (
            "Identical EEGNet, optimizer, crop order, stimulus head, and selection; "
        ...
        "tensor_archive": str(args.tensor_archive.resolve()),
        "tensor_archive_sha256": sha256(args.tensor_archive),
        "trial_uid_archive": str(args.trial_uid_archive.resolve()),
        "trial_uid_archive_sha256": sha256(args.trial_uid_archive),
