...
|---|---|---|
| Untouched confirmation dataset | Outcome-blind protocol locked; authorised AMIGOS data ingested; all prespecified analyses reported | Protocol, label-blind QC, ingestion, and 100-split/2,500-cell assignment generator tested; EULA/data access pending |
| Strong adapted model and seeds | LaBraM final-four-block adaptation and EEGNet run for five seeds on the confirmation design; failures retained | Resumable LaBraM, validation-selected EEGNet, and Ridge runners tested; five-seed same-data LaBraM grid running; AMIGOS runs await access |
| Training uncertainty | Five fold rotations are fully refit; crossed identity and training-seed uncertainty reported for the primary uniform mean | Joint identity/seed/rotation analysis tested; same-data outputs pending; AMIGOS rotations await access |
