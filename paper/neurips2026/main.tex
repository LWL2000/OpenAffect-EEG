...
12 task--setting grids, intervals for the mean matched EEG increment include
zero. These results do not establish equivalence or absence of EEG information;
they show that the observed total gains cannot be attributed to EEG under the
evaluated contracts. Crossed simulations verify intended positive-signal
behaviour but reveal serious undercoverage on the primary five-block topology;
the reported intervals are conditional diagnostics, not calibrated guarantees. A companion
...
\citep{li2021calibration,bendrich2022feature,liu2026eegemolib}. These efforts ask
whether data partitions, models, or representations generalize. Our unit of
...
The archived learning records quantify this qualification. Among 50 direct
and 50 residual EEGNet fits per task, respectively 12/10 Emo fits and 8/7
Urban fits select epoch one. Median selected epochs are 18/17.5 and 11/7.5.
Training objectives decrease in the median for all four groups. These real-task
records rule out an account in which every network simply remained at its
initial fit; they do not establish optimal tuning or a positive held-out
EEG signal. Appendix~\ref{app:revision-diagnostics} reports the reusable audit.
...
The primary full-grid summaries in Table~\ref{tab:primary-v9} tell the same
bounded story: all 12 task--setting intervals include zero under both interval
methods. No equivalence margin was prespecified, so this does not establish
...
error in both tasks under both interval constructions; other configurations are
...
data, not an additional confirmatory test. Conditional pointwise and
within-model dose-family intervals accompany every cell in the released CSV.}
...
interval. Two Urban LaBraM R1 cells have negative pointwise intervals, but
neither survives its within-model dose-family band. We retain all 48
...
are valid for EmoEEG-MC and 2,000 for Urban. Percentile intervals are primary;
an explicitly heuristic small-cluster t-normal analysis is a sensitivity check.
Both condition on fitted predictions and omit full retraining uncertainty.
Pointwise intervals and separately released dose-family simultaneous bands do
not correct all model comparisons.
...
For trial signal, percentile coverage of the uniform mean is 0.900 for Emo
and 0.927 for Urban; for participant-constant signal it is 0.903 and 0.970.
The worst cell is Emo's zero-dose participant-constant case: coverage is
0.193 (Monte Carlo interval $[0.153,0.242]$). Thus nominal 95\% coverage is
not established, even on the intended topology. The t-normal sensitivity
improves these mean-level checks but still undercovers that cell (0.887).
We retain both predeclared constructions as conditional diagnostics and make
no calibrated significance or equivalence claim. Earlier single-block tests
in Appendix~\ref{app:evaluator-validation} cannot override this failure.
...
and~\ref{tab:primary-v9} change the interpretation of actual EEG predictions
...
acceptance script, and workflow checker, verified on 9 September 2026.
